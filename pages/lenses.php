...
if (isset($_POST['insert'])) {
  if (isset($_POST['inputCamID'])){
    $id_camera = $_POST['inputCamID'];
  }
  $id_lens = $_POST['inputLensID'];
  $name = $_POST['inputName'];
  $format = $_POST['inputFormat'];
  $price = $_POST['inputPrice'];
  $type = $_POST['inputType'];
  $query = "CALL Create_Lensa('$format','$price','$id_lens','$type','$name');";
  $query .= "CALL Create_Detail_Tambahan('$id_camera','$id_lens');";
  ...
  header("Refresh:0; url=lenses.php");
}
...
if (isset($_POST['update'])) {
  ...
  $price = $_POST['inputPrice'];
  $type = $_POST['inputType'];
  $query = "UPDATE lensa SET NAMA_LENSA='$name',FORMAT_MOUNT_LENSA='$format',JENIS_LENSA='$type',HARGA_LENSA='$price' WHERE ID_LENSA='$id_lens';";
  $query .= "CALL Create_Detail_Tambahan('$id_camera','$id_lens');";
  $result = mysqli_multi_query($conn, $query);
  if ($result) {
    header("Location: lenses.php");
...
if ($action == "update") {
  $id_lens = $_GET['id'];
  $query = "SELECT * FROM lensa WHERE ID_LENSA = '$id_lens'";
  $result = mysqli_query($conn, $query);
  $row = mysqli_fetch_assoc($result);
  $id_lens = $row['ID_LENSA'];
  $name = $row['NAMA_LENSA'];
  $format = $row['FORMAT_MOUNT_LENSA'];
  $price = $row['HARGA_SEWA_LENSA'];
  $type = $row['JENIS_LENSA'];
}

if ($action == "delete") {
  $id_lens = $_GET['id'];
  $query = "CALL Delete_Lensa('$id_lens')";
  $result = mysqli_query($conn, $query);
  if ($result) {
    header("Location: lenses.php");
  } else {
        ...
        <div class="content-wrapper">
          <div class="row">
                ...
                <div class="card-body">
                  ...
                  <form class="forms-sample" method="POST">
                    <div class="form-group row">
                      ...
                      <div class="col-sm-9">
                        <select id="inputCamID" name="inputCamID" class="form-control" id="floatingSelect" aria-label="Floating label select example">
                          <option value="<?PHP echo $id_camera ?>">Pilih Kamera</option>
                          <?php
                          $sql = "SELECT * FROM kamera";
                          $result = mysqli_query($conn, $sql);
                          while ($row = mysqli_fetch_assoc($result)) {?>
                            <option value="<?php echo $row['ID_KAMERA'] ?>"><?php echo $row['NAMA_KAMERA'] ?></option>
                          <?php }
                          ?>
                        </select>
                      </div>
                    ...
                    <div class="form-group row">
                      <label for="inputLensID" class="col-sm-3 col-form-label">Lens ID</label>
                      <div class="col-sm-9">
                        <input type="text" class="form-control" id="inputLensID" name="inputLensID" placeholder="Lens ID" value="<?php echo $id_lens ?>">
                      </div>
                    </div>
                    <div class="form-group row">
                    ...
                    <div class="form-group row">
                      ...
                      <div class="col-sm-9">
                        <input type="text" class="form-control" id="inputType" name="inputType" placeholder="Type" value="<?php echo $type ?>">
                      </div>
                    </div>
                    <div class="form-group row">
            ...
            <div class="col-lg-6 grid-margin stretch-card">
              <div class="card">
                <div class="card-body">
                  <h4 class="card-title">Lens Table</h4>
                  <p class="card-description"> For checking any lens
                  </p>
                  <div class="table-responsive">
                    <table id="table_id" class="table table-hover">
                      ...
                      <tbody>
                        <?PHP
                        $query = "SELECT * FROM lensa";
                        $result = mysqli_query($conn, $query);
                        while ($row_select = mysqli_fetch_assoc($result)) {
                          echo "<tr>";
                          echo "<td>" . $row_select['ID_LENSA'] . "</td>";
                          echo "<td>" . $row_select['NAMA_LENSA'] . "</td>";
                          echo "<td>" . $row_select['FORMAT_MOUNT_LENSA'] . "</td>";
                          echo "<td>" . $row_select['HARGA_LENSA'] . "</td>";
                          echo "<td>" . $row_select['JENIS_LENSA'] . "</td>";
                          echo "<td>";
                          echo "<a href='lenses.php?action=update&id=" . $row_select['ID_LENSA'] . "' class='btn btn-primary m-1 btn-edit'><i class='mdi mdi-pencil'></i></a>";
                          echo "<a href='lenses.php?action=delete&id=" . $row_select['ID_LENSA'] . "' onclick='return confirm(\"Apakah anda yakin ingin menghapus data ?\")' class='btn btn-danger m-1'><i class='mdi mdi-delete'></i></a>";
                          echo "</td>";
                          echo "</tr>";
                        }
                        ?>
