...
if (isset($_POST['insert'])) {
  ...
  $query = "CALL Create_Fasilitas('$price','$id_fac','$name','$stock')";
...
if (isset($_POST['update'])) {
  ...
  $price = $_POST['inputPrice'];
  $stock = $_POST['inputStock'];
  $query = "UPDATE fasilitas SET NAMA_FASILITAS='$name',HARGA_FASILITAS='$price',STOK_FASILITAS='$stock' WHERE ID_FASILITAS='$id_fac'";
  $result = mysqli_query($conn, $query);
  if ($result) {
    header("Location: facility.php");
  } else {
...
if ($action == "update") {
  $id_fac = $_GET['id'];
  $query = "SELECT * FROM fasilitas WHERE ID_FASILITAS = '$id_fac'";
  $result = mysqli_query($conn, $query);
  $row = mysqli_fetch_assoc($result);
  $id_fac = $row['ID_FASILITAS'];
  $name = $row['NAMA_FASILITAS'];
  $price = $row['HARGA_FASILITAS'];
  $stock = $row['STOK_FASILITAS'];
}

if ($action == "delete") {
  $id_fac = $_GET['id'];
  $query = "CALL Delete_Fasilitas('$id_fac')";
  $result = mysqli_query($conn, $query);
  if ($result) {
    header("Location: facility.php");
  } else {
                ...
                <div class="card-body">
                  ...
                  <form class="forms-sample" method="POST">
                    <div class="form-group row">
                      ...
                      <div class="col-sm-9">
                        <input type="text" class="form-control" id="inputFacID" name="inputFacID" placeholder="Facility ID" value="<?php echo $id_fac ?>">
                      </div>
                    </div>
                    <div class="form-group row">
            ...
            <div class="col-lg-6 grid-margin stretch-card">
              <div class="card">
                <div class="card-body">
                  ...
                  <p class="card-description"> For checking any facility
                  </p>
                    ...
                    <table id='table_id' class="table table-hover display">
                      ...
                      <tbody>
                        ...
                        while ($row_select = mysqli_fetch_assoc($result)) {
                          ...
                          echo "<td>" . $row_select['NAMA_FASILITAS'] . "</td>";
                          echo "<td>" . $row_select['HARGA_FASILITAS'] . "</td>";
                          echo "<td>" . $row_select['STOK_FASILITAS'] . "</td>";
                          echo "<td>";
                          echo "<a href='facility.php?action=update&id=" . $row_select['ID_FASILITAS'] . "' class='btn btn-primary m-1 btn-edit'><i class='mdi mdi-pencil'></i></a>";
                          echo "<a href='facility.php?action=delete&id=" . $row_select['ID_FASILITAS'] . "' onclick='return confirm(\"Apakah anda yakin ingin menghapus data ?\")' class='btn btn-danger m-1'><i class='mdi mdi-delete'></i></a>";
                          echo "</td>";
                          echo "</tr>";
                        }
                        ?>
